...
# =============== 参数 ===============
HIST_LEN = 24       # 历史长度（一天）
PRED_LEN = 24       # 预测 D+1（一天）
STRIDE_DAYS = 24     # 两天一组，按天滑动
BATCH_SIZE = 64
EPOCHS = 150
LR = 5e-4
# site_id = 2
...
class PatchTST_D1(nn.Module):
    ...
    def forward(self, hist_x, fut_x):
        ...
        # 5) 95.2 gated fusion：fut = 0.5*gate*fut + cross_out
        gate = self.cross_gate(fut)  # [B, n_patch_future, 1]
        fut = 0.5 * gate * fut + cross_out
        # fut = fut + 0.5 * gate * cross_out
...
def run_one_site(site_id):
    ...
    y_scaled = df[[target]].values.astype(np.float32)
    ...
    def create_D1_samples(x, y, t, timestamps,
        ...
        for i in range(0, N - hist_len - pred_len + 1, step):
            ...
            hist_in = np.concatenate([x_hist, y_hist, t_hist], axis=-1)#95.1 enc_in_future = Cw
    ...
    os.makedirs("20260318patchPRE", exist_ok=True)
    d1_df.to_excel(f"20260318patchPRE/PREPatchTST_site_{site_id}_D+1_autoreg_earlystop.xlsx", index=False)

    with open(f"20260318patchPRE/PatchTST_site_{site_id}_D+1_data.pkl","wb") as f:
        pickle.dump({
            "pred": preds_inv,
            "true": trues_inv,
            "date": dates_test,
